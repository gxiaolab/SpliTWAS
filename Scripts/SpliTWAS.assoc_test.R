...
N = nrow(wgtlist)
out.tbl = data.frame( "PANEL" = rep(NA,N) , "FILE" = character(N) , "ID" = character(N) , "CHR" = numeric(N) , "P0" = numeric(N) , "P1" = numeric(N) ,"HSQ" = numeric(N) , "HSQ.P" = numeric(N), "BEST.GWAS.ID" = character(N) , "BEST.GWAS.Z" = numeric(N) , "EQTL.ID" = character(N) , "EQTL.R2" = numeric(N) , "EQTL.Z" = numeric(N) , "EQTL.GWAS.Z" = numeric(N) , "NSNP" = numeric(N) , "NWGT" = numeric(N) , "MODEL" = character(N) , "MODELCV.R2" = numeric(N) , "MODELCV.PV" = numeric(N) , "SpliTWAS.Z" = numeric(N) , "SpliTWAS.P" = numeric(N) , stringsAsFactors=FALSE )
...
sumstat$A1[ sum.missing ] = genos$bim[sum.missing,6]
sumstat$A2[ sum.missing ] = genos$bim[sum.missing,5]

# QC / allele-flip the input and output
qc = allele.qc( sumstat$A1 , sumstat$A2 , genos$bim[,6] , genos$bim[,5] )

# Flip Z-scores for mismatching alleles
sumstat$Z[ qc$flip ] = -1 * sumstat$Z[ qc$flip ]
sumstat$A1[ qc$flip ] = genos$bim[qc$flip,6]
sumstat$A2[ qc$flip ] = genos$bim[qc$flip,5]
...
#			cat( "WARNING :",  unlist(wgtlist[w,]) , "top eQTL is the best predictor for this gene, continuing with 2nd-best model (top eQTL results will also be reported)\n" )
		...
		if ( sum(wgt.matrix[, mod.best] != 0) == 0 ) {
		  cat( "WARNING : " , unlist(wgtlist[w,]) , names(cv.performance)[ mod.best ] , "had", length(cur.Z) , "overlapping SNPs, but none with non-zero expression weights with model ", colnames(cv.performance)[mod.best], "\n")
		  if (length(colnames(cv.performance)) > 2){
		    cat ("Trying different model\n")
		    mod.best = names( which.max(cv.performance[1,colnames(cv.performance) != "top1" & colnames(cv.performance) != colnames(cv.performance)[mod.best]]) )
		    mod.best = which( colnames(cv.performance) == mod.best)
		  } 
		}
	}
	...
	if ( sum(wgt.matrix[, mod.best] != 0) == 0 ) {
	  cat( "WARNING : " , unlist(wgtlist[w,]) , names(cv.performance)[ mod.best ] , "had", length(cur.Z) , "overlapping SNPs, but none with non-zero expression weights, two models used check weight files\n")
	  cur.FAIL = TRUE
	}
