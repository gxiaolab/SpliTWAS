...
weights.lasso = function( input , hsq , snp , out=NA ) {
	...
	arg = paste( opt$PATH_plink , " --allow-no-sex --bfile --memory 4000 " , input , " --lasso " , hsq , " --out " , out , sep='' )
	system( arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT )
...
arg = paste( opt$PATH_plink ," --allow-no-sex --bfile --memory 4000 ",opt$bfile," --pheno ",pheno.file," --keep ",pheno.file," --make-bed --out ",geno.file,sep='')
system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
...
# 1. generate GRM
arg = paste( opt$PATH_plink," --allow-no-sex --bfile --memory 4000 ",opt$tmp," --make-grm-bin --out ",opt$tmp,sep='')
system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
...
for ( i in 1:opt$crossval ) {
	...
	arg = paste( opt$PATH_plink ," --allow-no-sex --bfile  --memory 4000",opt$tmp," --keep ",cv.file,".keep --out ",cv.file," --make-bed",sep='')
	system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
...
for ( mod in 1:M ) {
	if ( models[mod] == "blup" ) {
		...
		wgt.matrix[ - which.max( wgt.matrix^2 ) ] = 0
	}
}
